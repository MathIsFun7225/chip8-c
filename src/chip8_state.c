...
int chip8_init_state(struct chip8_state *state, const struct chip8_config *config) {
    ...
    return chip8_reset_state(state, config);
}

int chip8_reset_state(struct chip8_state *state, const struct chip8_config *config) {
    uint16_t stack_size = state->stack_size;
    uint16_t *stack = state->stack;
    ...
    memcpy(&state->memory[FONT_MEMORY_OFFSET], lores_font, sizeof(lores_font));
    memcpy(&state->memory[FONT_MEMORY_OFFSET + LORES_FONT_LENGTH], hires_font, sizeof(hires_font));
